...
sed -i 's/PKG_HASH.*/PKG_SOURCE_VERSION:=latest/g' package/lean/qBittorrent/Makefile
...
find package/feeds/luci/*/* -name "*.htm" | xargs sed -i "s/\(<form .*action=\"\)<%=REQUEST_URI%>\(\" method=\"post\">\)/<script>document.write('\1'+document.URL+'\2')<\/script>/g"
find package/feeds/lienol/*/* -name "*.htm" | xargs sed -i "s/\(<form .*action=\"\)<%=REQUEST_URI%>\(\" method=\"post\">\)/<script>document.write('\1'+document.URL+'\2')<\/script>/g"
...
sed -i '$a /etc/nginx' package/base-files/files/lib/upgrade/keep.d/base-files-essential
