...
sed -i "s/\('SaveTime:string'\)/\1 'AutoUpdateTrackers:or(\"true\",\"false\"):true' 'CustomizeTrackersListUrl:string:https:\/\/trackerslist.com\/all.txt' 'AnnounceToAllTrackers:or(\"true\",\"false\"):true' 'trackerEnabled:or(\"true\",\"false\"):true' 'trackerPort:integer'/g" package/lean/luci-app-qbittorrent/root/etc/init.d/qbittorrent
...
sed -i '$a /usr/share/unblockneteasemusic/core' package/base-files/files/lib/upgrade/keep.d/base-files-essential
